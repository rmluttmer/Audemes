...
$(document).ready(function () {
    ...
    //Show the riddle's answer
    $('.showRiddleAnswer').click(function () {
        $(this).parent().children(".answer").toggleClass("visible");
    });

    //Add to Dashboard
    $('.addToDashboard').click(function () {
        var audemeID = $(this).attr("id");
        $.get("/formHandlers/addToDashboard.aspx", { audeme_id: audemeID });
        $(this).text("(In Locker)");
        $(this).unbind('click');
    ...
    $('.addGridToDashboard').click(function () {
        ...
        $.get("/formHandlers/addToDashboard_Grids.aspx", { audemeGrid_id: audemeGridID });
        $(this).text("(In Locker)");
        $(this).unbind('click');
        $(this).toggleClass("clickable");
    });
    ...
    $('#changeTextSize').click(function () {
        $.post("/formHandlers/changeTextSize.aspx", { fontSize: "0" });
    });
    ...
    $('.removeFromDashboard').click(function () {
        var audemeID = $(this).attr('id');
        $.get("/formHandlers/removeFromDashboard.aspx", { audeme_id: audemeID });
        $(this).text("(Removed)");
        $(this).unbind('click');
        $(this).toggleClass("clickable");
    });
    ...
    $('.removeGridFromDashboard').click(function () {
        var audemeGridID = $(this).attr('id');
        $.get("/formHandlers/removeFromDashboard_Grid.aspx", { audemeGrid_ID: audemeGridID });
        $(this).text("(Removed)");
        $(this).unbind('click');
        $(this).toggleClass("clickable");
    });
    ...
    $('.submitAudemeRating').click(function () {
        ...
        $.get("/formHandlers/rateAudeme.aspx", { audemeVar: audeme, conceptVar: concept, componentVar: component });
    });
    ...
    $('.audeme').click(function () {
        ...
        $.get("/formHandlers/Audeme_RecordPlayed.aspx", { audeme_ID: audeme });
    });
    ...
    $('#organizeAudemes').click(function () {
        ...
        if ($('#organizeAudemes').hasClass("sort")) {
            ...
            $.post("/formHandlers/reorderAudemeList.aspx", { orderedList: listPOST });
